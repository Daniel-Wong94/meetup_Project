...
const GroupAbout = () => {
  ...
  const isOrganizer =
    group?.organizerId && group?.organizerId === sessionUser?.id;

  const allMembers = group?.members && Object.values(group?.members);
  const members =
    allMembers &&
    allMembers.filter((member) => member?.Membership?.status !== "pending");
  const pending =
    allMembers &&
    allMembers.filter((member) => member?.Membership?.status === "pending");
      ...
      <div>
        <Switch>
          <Route path={`/discover/groups/${group.id}/about`}>
            <div className={styles.groupAboutContainer}>
              ...
              <div className={styles.rightSection}>
                ...
                <ProfilePicture
                  initials={
                    group?.Organizer?.firstName[0] +
                    group?.Organizer?.lastName[0]
                  }
                />
          ...
          <Route path={`/discover/groups/${group.id}/events`}>
            {group?.events && Object.values(group.events).length > 0 ? (
